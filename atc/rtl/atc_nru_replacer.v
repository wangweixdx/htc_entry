    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (nru_clear_all_used) begin
                ...
                // Only decay entries that have been accessed (ACTIVE or PROTECT).
                // FREE and IDLE entries retain their state so FREE still has
                // higher eviction priority than decayed entries.
                for (i = 0; i < 64; i = i + 1) begin
                    if (nru_used[i]) begin
                        nru_used[i]     <= 1'b0;
                        nru_not_last[i] <= 1'b0;
