...
module atc_ctrl (
    ...
    // Relook registered data from arbiter (aligned with arb_rl_grant)
    wire               arb_rl_pv;
    wire [19:0]        arb_rl_pasid;
    wire [15:0]        arb_rl_func_id;
    wire [63:0]        arb_rl_va;
    ...
    atc_req_arbiter u_arbiter (
        ...
        .rl_req_data_pv     (arb_rl_pv),
        .rl_req_data_pasid  (arb_rl_pasid),
        .rl_req_data_func_id(arb_rl_func_id),
        .rl_req_data_va     (arb_rl_va),
    ...
    wire               lu_nru_update_en;
    wire [5:0]         lu_nru_update_way;
    wire [1:0]         lu_nru_update_val;

    // Relook mux: route lookup or relook request to lookup engine
    assign lu_engine_req_valid = arb_lu_grant || arb_rl_grant;
    assign lu_engine_req_pv       = arb_rl_grant ? arb_rl_pv       : arb_lu_pv;
    assign lu_engine_pasid_sig    = arb_rl_grant ? arb_rl_pasid    : arb_lu_pasid;
    assign lu_engine_func_id_sig  = arb_rl_grant ? arb_rl_func_id  : arb_lu_func_id;
    assign lu_engine_va_sig       = arb_rl_grant ? arb_rl_va       : arb_lu_va;

    // Track request type and func_id through 4-stage pipeline
    reg [3:0]  rsp_is_lookup;
    reg [15:0] rsp_func_id_s0, rsp_func_id_s1, rsp_func_id_s2, rsp_func_id_s3;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            rsp_is_lookup    <= 4'd0;
            rsp_func_id_s0   <= 16'd0;
            rsp_func_id_s1   <= 16'd0;
            rsp_func_id_s2   <= 16'd0;
            rsp_func_id_s3   <= 16'd0;
        end else begin
            rsp_is_lookup    <= {rsp_is_lookup[2:0], arb_lu_grant};
            rsp_func_id_s0   <= lu_engine_func_id_sig;
            rsp_func_id_s1   <= rsp_func_id_s0;
            rsp_func_id_s2   <= rsp_func_id_s1;
            rsp_func_id_s3   <= rsp_func_id_s2;
        end
    end
    ...
    atc_lookup_engine u_lookup (
        ...
        .nru_update_en      (lu_nru_update_en),
        .nru_update_way     (lu_nru_update_way),
        .nru_update_val     (lu_nru_update_val),
    ...
    // Partition hash for insert set (64KB-aligned when prefetch enabled)
    wire [4:0] ins_set_idx;
    wire       ins_prefetch_en;
    wire [63:0] ins_hash_va;
    assign ins_prefetch_en = prefetch_enable[arb_ins_func_id[5:0]];
    assign ins_hash_va = ins_prefetch_en
        ? {arb_ins_va[63:20], arb_ins_va[19:16],
           arb_ins_va[19:16], arb_ins_va[11:0]}
        : arb_ins_va;
    assign ins_set_idx = partition_hash(ea_cfg_num_users,
        arb_ins_func_id[5:0], arb_ins_func_id, ins_hash_va);
    ...
    //=========================================================================
    // NRU Update Mux: merge lookup-hit and insert-write updates
    // Insert writes to a new (non-duplicate) way must mark it ACTIVE so NRU
    // won't select the same way again on the next insert.
    //=========================================================================
    wire ins_nru_update_en;
    assign ins_nru_update_en = do_insert_write && !dc_duplicate;

    assign ea_nru_update_en  = ins_nru_update_en ? 1'b1           : lu_nru_update_en;
    assign ea_nru_update_way = ins_nru_update_en ? ea_wr_way_idx  : lu_nru_update_way;
    assign ea_nru_update_val = ins_nru_update_en ? `NRU_ACTIVE    : lu_nru_update_val;
    ...
    wire rsp_prefetch_en;
    assign rsp_prefetch_en = prefetch_enable[rsp_func_id_s3[5:0]];

    assign dma_lu_rsp_valid  = lu_engine_rsp_valid && rsp_is_lookup[3];
    always @(*) begin
        dma_lu_rsp_hit_r     = lu_engine_rsp_exact_hit;
        dma_lu_rsp_pre_hit_r = lu_engine_rsp_pre_hit;
        if (!ats_enable[rsp_func_id_s3[5:0]]) begin
            dma_lu_rsp_hit_r     = 1'b0;
            dma_lu_rsp_pre_hit_r = 1'b0;
        end
        if (!rsp_prefetch_en) begin
            dma_lu_rsp_pre_hit_r = 1'b0;
        end
    end
    ...
    // Relook response
    assign dma_rl_rsp_valid  = lu_engine_rsp_valid && !rsp_is_lookup[3];
    ...
    // Per-function prefetch gating: when prefetch_enable[func_id]=0, all
    // prefetch outputs are forced to 0 for that function.
    assign flr_done            = invh_flr_done;
    assign prefetch_rsp_valid  = lu_engine_rsp_prefetch_hit & {16{lu_engine_rsp_valid && rsp_prefetch_en}};
    assign prefetch_hit        = lu_engine_rsp_prefetch_hit & {16{rsp_prefetch_en}};
